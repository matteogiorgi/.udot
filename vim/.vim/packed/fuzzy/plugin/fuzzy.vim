...
function s:FuzzyFind()
    ...
    call system('$TERMINAL -T __fuzzy__ -e $HOME/.vim/packed/fuzzy/plugin/fuzzyfind s:folder')
...
function s:FuzzyJump()
    let s:folder = expand("%:p:h")
    call system('$TERMINAL -T __fuzzy__ -e $HOME/.vim/packed/fuzzy/plugin/fuzzyjump s:folder')
...
function s:FuzzyGit()
    call system('$TERMINAL -T __fuzzy__ -e $HOME/.vim/packed/fuzzy/plugin/fuzzygit')
    redraw!
endfun
