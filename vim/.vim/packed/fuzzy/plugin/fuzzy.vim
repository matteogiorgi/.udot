...
function s:FuzzyFind()
    ...
    call system('$TERMINAL --title __600x200__ -e $HOME/.vim/packed/fuzzy/plugin/fuzzyfind s:folder')
...
function s:FuzzyJump()
    let s:folder = expand("%:p:h")
    call system('$TERMINAL --title __600x200__ -e $HOME/.vim/packed/fuzzy/plugin/fuzzyjump s:folder')
